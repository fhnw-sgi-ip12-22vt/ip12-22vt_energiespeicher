package ch.fhnw.clashofenergiespeicher.catalog;

import ch.fhnw.clashofenergiespeicher.catalog.applications.Ads1115_App;
import ch.fhnw.clashofenergiespeicher.catalog.applications.LedStrip_App;
import ch.fhnw.clashofenergiespeicher.catalog.applications.Potentiometer_App;
import ch.fhnw.clashofenergiespeicher.catalog.applications.SimpleButton_App;
import ch.fhnw.clashofenergiespeicher.catalog.components.helpers.SingletonAppHelper;
...
public final class Launcher implements Runnable {
    /**
     * This list must contain all applications which should be executable through
     * the launcher.
     * Each class instance must implement the Application interface and gets
     * automatically added as a subcommand.
     */
    ...
    /**
     * Demo mode will keep the launcher running forever, allowing the consecutive
     * execution of several applications.
     * This value gets dynamically set to {@code false} when the user selects to
     * exit the launcher in the interactive menu.
     */
    @CommandLine.Option(names = { "-d",
            "--demo" }, description = "Enable demo mode to run multiple applications consecutively")
    ...
    /**
     * Main application entry point which executes the launcher and exits
     * afterwards.
     *
    ...
    /**
     * Creates a new launcher with an eagerly initialized Pi4J context and PicoCLI
     * instance.
     * All applications specified in APPLICATIONS are being automatically
     * registered.
     *
    ...
    /**
     * When the user does not specify any sub-command and therefore does not
     * explicitly state which application should be launched,
     * a manual console based selection menu of all known applications will be
     * shown. This allows to easily run any desired application
    ...
    public void run() {
        ...
        // This loop will either run only once or forever, depending on the state of
        // `demoMode`
        do {
     ...
     * until a valid choice via stdin has been made.
     * If an invalid input occurs, this method will keep retrying until a valid
    ...
    /**
     * Uses PicoCLI to parse the given command line arguments and returns an
     * appropriate POSIX exit code.
     *
     * @param args List of command line arguments to parse, usually provided by the
     *             main entrypoint of a Java application
    ...
    /**
     * Registers all applications as sub-commands by wrapping them with an
    ...
    /**
     * Helper class which wraps around an application to implement the Runnable
     * interface.
     * This can not be done directly in the interface as it will need to pass the
     * Pi4J context of the parent class.
     */
    @Command()
