package ch.fhnw.clashofenergiespeicher.catalog.components;

import ch.fhnw.clashofenergiespeicher.catalog.components.helpers.ContinuousMeasuringException;
...
public class Ads1115 extends Component {
    ...
    /**
     * The Config register is used to change the ADS1115 operating modes and query
    ...
    /**
     * Creates a new AD converter component with default bus 0x01, device bus
     * address 0x48 (GND)
     ...
     *                      voltage)
     * @param readFrequency read frequency to get new value from device, must be
     *                      lower than 1/2
     *                      sampling rate of device
     */
    ...
    public void startSlowContinuousReading(int channel, double threshold, int readFrequency) {
        ...
        // only start continuous Reading if it is not already running because of other
        // component
        if (!continuousReadingActive) {
            continuousReadingActive = true;
    ...
    /**
     * Return GAIN object with bit structure for configuration and resolution (gain
     * per bit)
     *
     * @return GAIN object
     */
    ...
    /**
     * Sets or disables the handler for the onValueChange event from continuous slow
     * read.
     * This event gets triggered whenever the analog value
     * from the device changes.
     * Only a single event handler can be registered at once.
    ...
    /**
     * Sends configuration for continuous reading to device, updates actual value
     * from analog input
    ...
    /**
     * Sends, for each channel, a request to device and wait for response. Enters
     * all responses in actualValue array.
        ...
        if (readFrequency * numberOfChannels * 2 < dr.getSpS()) {
            ...
            new Thread(() -> {
                while (continuousReadingActive) {
                    ...
                    result[0] = readSingleShot(
                            CONFIG_REGISTER_TEMPLATE | MUX.AIN0_GND.getMux() | MODE.SINGLE.getMode());
                    ...
                    for (int i = 0; i < numberOfChannels; i++) {
                        if (oldValue[i] - thresholdDigits > result[i] || oldValue[i] + thresholdDigits < result[i]) {
                            logDebug("New event triggered on value change, old value: " + oldValue[i] + " , new value: "
                                    + result[i]);
                            oldValue[i] = result[i];
     ...
     * The ADS1115 has one address pin, ADDR, that configures the I2C address of the
     * device. This pin can be
     * connected to GND, VDD, SDA, or SCL, allowing for four different addresses to
     * be selected with one pin.
     * Use the GND, VDD and SCL addresses first. If SDA is used as the device
     * address, hold the SDA line low
     * for at least 100 ns after the SCL line goes low to make sure the device
     * decodes the address correctly
    ...
    /**
     * A programmable gain amplifier (PGA) is implemented before the ΔΣ ADC of the
     * ADS1115. The
     * full-scale range is configured by bits PGA[2:0] in the Config register and
     * can be set to ±6.144 V, ±4.096 V,
     ...
     * clears after conversions are within the margin of the upper and lower
     * threshold
     * values.
     * <p>
