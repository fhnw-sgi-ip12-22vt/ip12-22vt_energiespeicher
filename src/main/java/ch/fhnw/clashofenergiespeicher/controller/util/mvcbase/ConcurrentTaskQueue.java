 ...
 *
 * Execution is asynchronous - possibly in a different thread - but the sequence
 * is kept stable, such that
 * for all tasks A and B: if B is submitted after A, B will only be executed
 * after A is finished.
 *
 * New tasks can be submitted while tasks are running.
 *
 * Task submission itself is supposed to be thread-confined,
 * i.e. creation of the ConcurrentTaskQueue and task submission is expected to
